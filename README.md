...
[Ameath](https://gitee.com/lzy-buaa-jdi/ameath) 以 Rust 重寫而成。她會在螢幕上遊蕩、
...
- 💬 **AI 聊天陪伴** — 雙擊寵物即可開啟聊天視窗，用她的語氣跟你對話；即使視窗關閉，
  狀態氣泡也會顯示她正在思考或已有新回覆
- 🔌 **自由選擇 AI 提供者** — 支援 OpenAI、Anthropic（Claude）、本機 Ollama 模型，
  或內建的離線展示模式；預設為關閉，啟用雲端服務前會先徵求你的同意
- 🛠️ **工具調用代理** — 本機 Ollama 模型可在回覆時讀取檔案、執行指令、搜尋網路，
  並有權限機制確認高風險操作；完整說明見 [`AI_FEATURES.md`](AI_FEATURES.md)
