...
- 💬 **AI 채팅 컴패니언** — 펫을 더블클릭하면 채팅 창이 열리고, 캐릭터의 말투로
  대화할 수 있습니다. 창을 닫아도 상태 말풍선으로 생각 중이거나 새 답장이 왔는지
  확인 가능
- 🔌 **원하는 AI 제공자 선택** — OpenAI, Anthropic(Claude), 로컬 Ollama 모델,
  또는 내장된 오프라인 데모 모드 중 선택 가능. 기본값은 꺼짐이며, 클라우드 서비스
  사용 전에는 한 번 동의를 구함
- 🛠️ **도구 호출 에이전트** — 로컬 Ollama 모델은 답변 중에 파일 읽기, 명령 실행,
  웹 검색을 수행할 수 있으며, 위험한 작업은 권한 확인을 거칩니다. 자세한 내용은
  [`AI_FEATURES.md`](AI_FEATURES.md)를 참고하세요
