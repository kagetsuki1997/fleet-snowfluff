...
[Ameath](https://gitee.com/lzy-buaa-jdi/ameath) 用 Rust 重写而成。她会在屏幕上游荡、
...
- 💬 **AI 聊天陪伴** — 双击宠物即可打开聊天窗口，用她的语气跟你对话；即使窗口关闭，
  状态气泡也会显示她正在思考或已有新回复
- 🔌 **自由选择 AI 提供方** — 支持 OpenAI、Anthropic（Claude）、本地 Ollama 模型，
  或内置的离线演示模式；默认为关闭，启用云端服务前会先征求你的同意
- 🛠️ **工具调用代理** — 本地 Ollama 模型可在回复时读取文件、执行命令、搜索网络，
  并有权限机制确认高风险操作；完整说明见 [`AI_FEATURES.md`](AI_FEATURES.md)
...
另外，NixOS／Nix 用户可通过本项目提供的 flake 直接构建或运行（`nix build`／
`nix run`），或作为 flake input 加入自己的系统配置，详见
