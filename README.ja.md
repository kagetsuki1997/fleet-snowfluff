...
- 💬 **AI チャットコンパニオン** — ペットをダブルクリックするとチャットウィンドウが
  開き、キャラクターになりきって会話できます。ウィンドウを閉じていても、考え中や
  返信待ちはステータスバブルで表示
- 🔌 **お好みの AI プロバイダーを選択** — OpenAI・Anthropic（Claude）・ローカルの
  Ollama モデル・内蔵のオフラインデモモードに対応。デフォルトでは無効で、クラウド
  サービス利用前には一度だけ確認を表示
- 🛠️ **ツール呼び出しエージェント** — ローカルの Ollama モデルは返信の一部として
  ファイルの読み取り、コマンドの実行、ウェブ検索を行えます。リスクのある操作には
  許可確認の仕組みがあります。詳細は [`AI_FEATURES.md`](AI_FEATURES.md) を参照
