...
- Treat Aemeath's own chat transcript as the canonical conversation and the CLI session as a cache of it. A resumed CLI session receives the latest message plus any turns it never saw (in `mix` mode another provider may have answered some in between); a session started without a usable resume (first message, stale-resume retry, escalation) receives a compact preamble rendered from the recent transcript.
- Persist each conversation's CLI session references in a sidecar file next to its chat log (`<ts>_<id>.sessions.json`), keyed by profile, recording the session id, the working directory it was created under, and how many transcript messages it is known to hold, and reload it when the conversation is resumed after a restart. A stored entry is used only if its recorded working directory matches the one about to be used.
