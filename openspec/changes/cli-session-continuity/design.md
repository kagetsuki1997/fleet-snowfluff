...
**5. A stored session is used only if its recorded working directory equals the one about to be used.** As far as is known, Claude Code scopes its sessions by working directory, so an id created under one directory may not resolve under another. This is unverified against the CLI, but recording the directory and comparing costs nothing and is safe if the assumption is wrong: at worst a valid session is discarded and the preamble restores context.
...
- [Resume-scoped-by-working-directory is an unverified assumption] → recorded and compared regardless (Decision 5); a wrong assumption only causes extra invalidation.
